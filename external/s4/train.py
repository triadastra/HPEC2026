...
class SequenceLightningModule(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        x = self._mask_grid_output(x, z)  # S4ND: zero absent (non-kept) grid cells pre-loss
        return x, y, w

    def _mask_grid_output(self, x, z):
        """S4ND fairness fix: zero predictions at structurally-absent (non-kept)
        grid cells BEFORE the loss, so TRAINING + early-stopping optimize only the
        903 kept combos (matching the masked eval in s4_eval.py). Absent targets are
        already 0, so masked cells contribute 0 loss and 0 gradient; val-loss stays
        monotonic in the kept-combo loss, so checkpoint selection is unchanged.
        No-op unless the dataset exposes kept_mask (i.e., the S4ND grid datasets)."""
        # State-flat grid (s4nd_2d_state): output is (B, S, vw) and the valid
        # states vary per sample, so masking is driven by the per-sample
        # valid_mask carried in the batch (not a static kept_mask).
        if x.dim() == 3 and isinstance(z, dict) and "valid_mask" in z:
            vm = z["valid_mask"].to(device=x.device, dtype=x.dtype)  # (B, S)
            return x * vm.unsqueeze(-1)
        km = getattr(getattr(self, "dataset", None), "kept_mask", None)
        if km is None:
            return x
        m = torch.as_tensor(km, device=x.device, dtype=x.dtype)  # (S,C,2) for 2d/3d; (S,G) for the state grid
        if x.dim() == 4 and m.dim() == 2:     # state grid: (B, S, G, vw), mask (S, G)
            return x * m[None, :, :, None]
        if x.dim() == 5:                      # 3d grid: (B, S, C, F, vw)
            return x * m[None, ..., None]
        if x.dim() == 4:                      # 2d per-flow: (B, S, C, vw)
            fids = z.get("flow_ids") if isinstance(z, dict) else None
            if fids is None:
                return x
            fids = fids.reshape(fids.shape[0], -1)[:, 0].long()
            sel = m[:, :, fids].permute(2, 0, 1)             # (B, S, C)
            return x * sel.unsqueeze(-1)
        return x
