...
class InformerDataset(Dataset):
    ...
    def n_tokens_time(self):
        if self.freq == 'h':
            ...
            # Monthly: MonthOfYear only (for timeenc=1 this still sets length=1)
            return [12]
        else:
            raise NotImplementedError
...
class _Dataset_AGG(InformerDataset):
    def __init__(self, data_path="custom_data_org_sum.csv", freq="M", **kwargs):
        super().__init__(data_path=data_path, freq=freq, **kwargs)

    def _process_columns(self, df_raw):
        # Rename time column to expected 'date' and keep available numeric columns
        df = df_raw.rename(columns={"Time": "date"})
        preferred_cols = [
            "date",
            "Total Vessel Value ($US)",
            "Total Containerized Vessel Value ($US)",
            "Total Vessel SWT (kg)",
            "Total Containerized Vessel SWT (kg)",
        ]
        existing = [c for c in preferred_cols if c in df.columns]
        return df[existing]


class AGG(InformerSequenceDataset):
    _name_ = "agg"

    _dataset_cls = _Dataset_AGG

    init_defaults = {
        "size": [36, 12, 12],  # seq_len, label_len, pred_len
        "features": "M",      # multivariate forecasting over all provided columns
        "target": "Total Vessel Value ($US)",
        "variant": 0,
        "scale": True,
        "inverse": False,
        "timeenc": 1,
