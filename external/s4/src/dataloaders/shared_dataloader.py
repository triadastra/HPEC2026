...
# -----------------------------
# Normalization (hierarchical backoff)
# -----------------------------
def compute_series_stats(train_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    s_exact.columns = ['State','Commodity','Import/Export','val_mean','val_std','wt_mean','wt_std']

    s_cf = (
        train_df.groupby(['Commodity','Import/Export'])[[VALUE_CANON, WEIGHT_CANON]]
                .agg(['mean','std']).reset_index()
    )
    s_cf.columns = ['Commodity','Import/Export','val_mean','val_std','wt_mean','wt_std']

    s_c = (
        train_df.groupby(['Commodity'])[[VALUE_CANON, WEIGHT_CANON]]
                .agg(['mean','std']).reset_index()
    )
    s_c.columns = ['Commodity','val_mean','val_std','wt_mean','wt_std']

    g = train_df[[VALUE_CANON, WEIGHT_CANON]].agg(['mean','std'])
    g = {
        'val_mean': float(g.loc['mean', VALUE_CANON]),
        'val_std':  float(g.loc['std',  VALUE_CANON]) or 1.0,
        'wt_mean':  float(g.loc['mean', WEIGHT_CANON]),
        'wt_std':   float(g.loc['std',  WEIGHT_CANON]) or 1.0,
    }

    for df in (s_exact, s_cf, s_c):
        df['val_std'] = df['val_std'].replace(0, 1.0)
        df['wt_std']  = df['wt_std'].replace(0, 1.0)

    return {'exact': s_exact, 'cf': s_cf, 'c': s_c, 'global': g}

def apply_stats_with_backoff(df: pd.DataFrame, stats: Dict[str, Any], clip: float = 8.0) -> pd.DataFrame:
    out = df.sort_values(KEYS + ['Time']).copy()

    # Level 1: exact
    out = out.merge(stats['exact'], on=KEYS, how='left')

    # Level 2: (Commodity, Flow)
    mask = out['val_mean'].isna() | out['val_std'].isna()
    if mask.any():
        cf = stats['cf'].copy().add_prefix('cf_')
        cf = cf.rename(columns={'cf_Commodity':'Commodity', 'cf_Import/Export':'Import/Export'})
        out = out.merge(cf, on=['Commodity','Import/Export'], how='left')
        for base, back in [('val_mean','cf_val_mean'), ('val_std','cf_val_std'),
                           ('wt_mean','cf_wt_mean'), ('wt_std','cf_wt_std')]:
            out[base] = out[base].fillna(out[back])

    # Level 3: Commodity
    mask = out['val_mean'].isna() | out['val_std'].isna()
    if mask.any():
        c = stats['c'].copy().add_prefix('c_').rename(columns={'c_Commodity':'Commodity'})
        out = out.merge(c, on=['Commodity'], how='left')
        for base, back in [('val_mean','c_val_mean'), ('val_std','c_val_std'),
                           ('wt_mean','c_wt_mean'), ('wt_std','c_wt_std')]:
            out[base] = out[base].fillna(out[back])

    # Level 4: global
    g = stats['global']
    out['val_mean'] = out['val_mean'].fillna(g['val_mean'])
    out['val_std']  = out['val_std'].fillna(g['val_std']).replace(0, 1.0)
    out['wt_mean']  = out['wt_mean'].fillna(g['wt_mean'])
    out['wt_std']   = out['wt_std'].fillna(g['wt_std']).replace(0, 1.0)

    # Apply z-score
    out[VALUE_CANON]  = (out[VALUE_CANON]  - out['val_mean']) / out['val_std']
    out[WEIGHT_CANON] = (out[WEIGHT_CANON] - out['wt_mean'])  / out['wt_std']

    # Clip extremes
    if clip is not None:
        out[VALUE_CANON]  = out[VALUE_CANON].clip(-clip, clip)
        out[WEIGHT_CANON] = out[WEIGHT_CANON].clip(-clip, clip)

    # Cleanup
    drop_cols = [c for c in out.columns if c.startswith('cf_') or c.startswith('c_')]
    out = out.drop(columns=drop_cols)
    return out

def _sanity_check_normalization(df: pd.DataFrame, tag: str):
    vm, vs = df[VALUE_CANON].mean(), df[VALUE_CANON].std()
    wm, ws = df[WEIGHT_CANON].mean(), df[WEIGHT_CANON].std()
    print(f"[{tag}] z-stats ~ Value mean={vm:.3f} std={vs:.3f} | Weight mean={wm:.3f} std={ws:.3f}")
