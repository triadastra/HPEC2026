...
import numpy as np
import torch
# torchaudio / torchvision are only needed by the audio/vision datasets, which
# we don't use for trade forecasting and which are painful to ABI-match against
# this torch build. Make them optional so the trade datasets (ts/et/tx) import
# cleanly on a GPU box without those packages.
try:
